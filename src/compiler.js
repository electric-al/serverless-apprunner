...
const compileVpcConnector = (images, ret, serverless) => {
  ...
  ret.Resources.AppRunnerServiceVpcConnector = {
    ...
    Properties: {
      SecurityGroups: serverless.variables.service.provider.vpc.securityGroupIds,
      Subnets: serverless.variables.service.provider.vpc.subnetIds,
      VpcConnectorName: `${serverlessServiceName}-${stage}-vpc-connector`
    }
  };
}

const compileServiceInstanceRole = (serviceId, service, images, ret, serverless) => {
  const stage = serverless.variables.options.stage;
  const serverlessServiceName = serverless.variables.service.service;
...
const compileService = (serviceId, service, images, ret, serverless) => {
  ...
  const instanceRoleName = compileServiceInstanceRole(serviceId, service, images, ret, serverless);
  ...
  ret.Resources[serviceCfId]= {
    ...
    Properties: {
      ...
      SourceConfiguration: {
        AuthenticationConfiguration: { AccessRoleArn: { 'Fn::GetAtt': ['AppRunnerImageAccessRole', 'Arn'] } },
      ...
      NetworkConfiguration: {
        EgressConfiguration: {
          EgressType: 'VPC',
          VpcConnectorArn: { 'Fn::GetAtt': ['AppRunnerServiceVpcConnector', 'VpcConnectorArn'] }
        }
      },
